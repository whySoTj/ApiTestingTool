...
public class CreatingTestCase {
    ...
    public ArrayList<TestResult> generateTestcasesForString(BodyData data, String attribute,String comparator, String replaceValue) {
     ...
     public ArrayList<TestResult> generateTestcasesForStringLength(BodyData data, String attribute,String comparator, int maxLength) {
        ...
        if (comparator.equals("<")) {
             ...
             System.out.println(newString+ "  "+ maxLength);

            replaceJsonStringValue(Json1, attribute, newString);
            replaceJsonStringValue(Json2, attribute, newString+"aa");

            if (apiCall.apiCalls(data,Json1).getStatusCode().is2xxSuccessful()) {
                result.add(new TestResult(true,"Length for "+attribute+" is "+ (Json1.get(attribute)).toString().length()));
            } else
                result.add(new TestResult(false,"Length for "+attribute+" is "+ (Json1.get(attribute)).toString().length()));

            if (apiCall.apiCalls(data,Json2).getStatusCode().is2xxSuccessful()) {
                result.add(new TestResult(false,"Length for "+attribute+" is "+ (Json2.get(attribute)).toString().length()));
            } else
                result.add(new TestResult(true,"Length for "+attribute+" is "+ (Json2.get(attribute)).toString().length()));
    ...
    public ArrayList<TestResult> generateTestcases(BodyData data, String attribute, String comparator, int value) {
        ...
        if (comparator.equals(">")) {
            ...
            replaceJsonIntegerValue(Json2, attribute, value + 1);
              ...
              System.out.println(apiCall.apiCalls(data,Json1).getStatusCode());
            if (apiCall.apiCalls(data,Json1).getStatusCode().is2xxSuccessful()) {
                
                result.add(new TestResult(false,"Value of "+attribute+" is " +(value-1)));
            } else
               {System.out.println(Json1 ); result.add(new TestResult(true,"Value of "+attribute+" is " +(value-1) ));}
    ...
    private static void replaceJsonStringValue(JsonNode node, String attribute, String replaceValue) {
        if (node.isObject()) {
            node.fields().forEachRemaining(entry -> {
                String fieldName = entry.getKey();
                JsonNode value = entry.getValue();
